...
for tool_name in tool_names:
    for video_id in video_ids:
        ...
        if pathlib.Path(alignment_path).is_file() and pathlib.Path(init_policy_path).is_file():
            alignment_params = pickle.load(open(alignment_path, 'rb'))
            for seed in seeds:
                reward = evaluate(None, str(alignment_path), str(ddp_q_path), str(init_policy_path), visualize=visualize,
                                  yaml_file=yaml_file_path, seed=seed, tool=tool_name, verbose=True)
                avg_reward += reward / len(seeds)
        if verbose:
            print(f"Final reward = {round(avg_reward, 2)}")
        result_list.append([tool_name, video_id, 'Initial policy', alignment_params['trajectory_scale'], seeds,
                            avg_reward])
print(result_list)
